...
export default function SwapBox(props) {
  ...
  const maxFromTokenInUSD = useMemo(() => {
    const value = fromTokenInfo.maxUsdqAmount
      ...
      .div(expandDecimals(1, USDQ_DECIMALS));
  ...
  let entryMarkPrice;
  let markPriceDisplayDecimals = USD_DISPLAY_DECIMALS;
  let exitMarkPrice;
  if (toTokenInfo) {
    ...
    markPriceDisplayDecimals = toTokenInfo.displayDecimals;
  }
  ...
  const getSwapError = () => {
    ...
    if (!isMarketOrder) {
      ...
      if (currentRate && currentRate.lt(triggerRatio)) {
        return [`Price ${triggerRatioInverted ? "below" : "above"} Mkt. Price`];
      }
    }
    ...
    if (
      ...
      fromTokenInfo.usdqAmount &&
      fromTokenInfo.maxPrice
    ) {
      const usdqFromAmount = adjustForDecimals(fromUsdMin, USD_DECIMALS, USDQ_DECIMALS);
      const nextUsdqAmount = fromTokenInfo.usdqAmount.add(usdqFromAmount);
  ...
  const getLeverageError = () => {
    ...
    if (isLong) {
      ...
      if (fromTokenAddress !== toTokenAddress) {
        ...
        if (toToken && toTokenAddress !== USDQ_ADDRESS) {
        ...
        if (
          ...
          const nextUsdqAmount = fromTokenInfo.usdqAmount.add(usdqFromAmount);
          if (nextUsdqAmount.gt(fromTokenInfo.maxUsdqAmount)) {
            return [`${fromTokenInfo.symbol} pool exceeded, try different token`, true, "MAX_USDQ"];
          }
        }
      }
  ...
  const getPrimaryText = () => {
    ...
    if (isLong) {
      ...
      if (indexTokenInfo && indexTokenInfo.minPrice) {
        ...
        if (fromTokenAddress === USDQ_ADDRESS && nextToAmountUsd.lt(fromUsdMin.mul(98).div(100))) {
          return "High USDQ Slippage, Long Anyway";
        }
      }
    ...
    Api.callContract(chainId, contract, method, params, {
      ...
      .finally(() => {
        ...
        fireEvent("trade", {
          ...
          asset_amount: formatAmount(fromAmount, fromToken.decimals, 4, true),
  ...
  const increasePosition = async () => {
    ...
    if (fromAmount && fromAmount.gt(0) && fromTokenAddress === USDQ_ADDRESS && isLong) {
  ...
  if (isSwap) {
    if (fromAmount) {
      ...
      if (feeBasisPoints !== undefined) {
        ...
        const feeTokenPrice =
          fromTokenInfo.address === USDQ_ADDRESS ? expandDecimals(1, USD_DECIMALS) : fromTokenInfo.maxPrice;
          ...
          <React.Fragment>
            ...
            <div className="Exchange-swap-section">
              <div className="Exchange-swap-section-top">
                ...
                {toBalance && isSwap && (
                  <div className="align-right">Balance: {formatAmount(toBalance, toToken.decimals, 4, true)}</div>
                )}
                {(isLong || isShort) && hasLeverageOption && (
                  <div className="align-right">Leverage: {parseFloat(leverageOption).toFixed(2)}x</div>
                )}
              </div>
              <div className="Exchange-swap-section-bottom">
                <div>
          ...
          <div className="Exchange-swap-section">
            <div className="Exchange-swap-section-top">
              ...
              <div
                className="align-right clickable"
                onClick={() => {
                  setTriggerPriceValue(formatAmountFree(entryMarkPrice, USD_DECIMALS, markPriceDisplayDecimals));
                }}
              >
                Mark: {formatAmount(entryMarkPrice, USD_DECIMALS, markPriceDisplayDecimals, true)}
              </div>
            </div>
            <div className="Exchange-swap-section-bottom">
              <div className="Exchange-swap-input-container">
          ...
          <div className="Exchange-leverage-box">
            ...
            <div className="Exchange-info-row">
              ...
              <div className="align-right">
                ...
                {nextAveragePrice && `$${formatAmount(nextAveragePrice, USD_DECIMALS, markPriceDisplayDecimals, true)}`}
                {!nextAveragePrice && `-`}
              </div>
            </div>
            <div className="Exchange-info-row">
              ...
              <div className="align-right">
                ...
                {toAmount &&
                  ...
                  `$${formatAmount(displayLiquidationPrice, USD_DECIMALS, markPriceDisplayDecimals, true)}`}
                ...
                renderContent={() => {
                  return (
                    <>
                      ...
                      {formatAmount(entryMarkPrice, USD_DECIMALS, markPriceDisplayDecimals, true)} USD with a max
                ...
                renderContent={() => {
                  return (
                    <>
                      ...
                      {formatAmount(entryMarkPrice, USD_DECIMALS, markPriceDisplayDecimals, true)} USD.
                      <br />
                      <br />
